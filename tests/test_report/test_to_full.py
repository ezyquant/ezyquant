...
def test_generate_full_with_dividend(cash_series: pd.Series, trade_df: pd.DataFrame):
    ...
    sbr.to_full(with_dividend=True)
...
def test_generate_full_no_trade(cash_series: pd.Series):
    sbr = SETBacktestReport(
        initial_capital=1.0,
        pct_commission=0.0,
        pct_buy_slip=0.0,
    ...
    sbr.to_full(with_dividend=False)
